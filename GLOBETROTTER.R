...
## usage:   (1) R CMD SHLIB -o GLOBETROTTERCompanion.so GLOBETROTTERCompanion.c -lz    (do only once)
## then:    (2) R < GLOBETROTTER.R parameter.infile.list samplefiles.infile.list recomratefiles.infile.list --no-save > output.out
 
## example1 -- SOURCE/DATE/PROPORTION ESTIMATION:  R < GLOBETROTTER.R example/BrahuiYorubaSimulation.paramfile.txt example/BrahuiYorubaSimulation.samplesfile.txt  example/BrahuiYorubaSimulation.recomfile.txt --no-save > output.out
## example2 -- DATE-BOOTSTRAPPING FOR CIs:  R < GLOBETROTTER.R example/BrahuiYorubaSimulation.paramfileII.txt example/BrahuiYorubaSimulation.samplesfile.txt  example/BrahuiYorubaSimulation.recomfile.txt --no-save > output.out
...
usage=function()
{
	...
	print(noquote("bootstrap.date.ind: [0,1]"))
...
if ((length(admix.curves.ind)!=1)||(admix.curves.ind!=0 && admix.curves.ind!=1)) error.input.message(param.infile)
...
if (length(save.file.bootstraps)!=1) error.input.message(param.infile)
...
options(scipen=999)
dyn.load("GLOBETROTTERCompanion.so")
...
for (i in 1:length(donor.pops.all))
  {
    ...
    predmat.orig=cbind(predmat.orig,apply(matrix(as.double(unlist(probs[,2:dim(probs)[2]][,match.i])),nrow=dim(probs)[1]),1,sum))
  }
...
close(readfile)
readfile=file(recomrates.filein,open="r")
for (i in 1:num.recomfiles)
{
	line2=scan(readfile,nlines=1,what='char',quiet=TRUE)
	x=read.table(line2,as.is=T,head=T)
	n.x=dim(x)[1]
	total.cM.i=sum(as.double(x[1:(n.x-1),2])*(as.double(x[2:n.x,1])-as.double(x[1:(n.x-1),1])))*100
	if (total.cM.i<grid.range[1])
	{
		print(paste("You have specified not to fit any chunks separated by less than ",grid.range[1],"cM ('grid.range'), but painting input file ",i," has total cM length = ",round(total.cM.i,4),"cM, which is less than this. Exiting....",sep=''))
		q(save='no')
	}
}
close(readfile)	
...
while(!is.na(line2[1]))
{
    	...
    	if (!is.na(line2[1]))
	{
		...
		{
			if (is.element(line2[3],recipient.label.vec))
			{
				recipient.label.vec.new=c(recipient.label.vec.new,line2[3])
				rec.hap.count=rec.hap.count+1
			}
		}
		count=count+1
	}
...
close(readfile)
recipient.label.vec=unique(recipient.label.vec.new)
...
{
	print(paste("Number of recipient individuals in ",as.character(read.table(samples.filein,nrow=1,as.is=TRUE))," does not match ploidy! Exiting....",sep=''))
	q(save='no')
}
if (ninds<=0)
{
	print(paste("No recipient individuals in ",as.character(read.table(samples.filein,nrow=1,as.is=TRUE)),"! Exiting....",sep=''))
	q(save='no')
}
...
if (prop.ind==1)
{
	...
	{
		print("A SINGLE SURROGATE POP FITS MIXTURE AFTER 0 ITERATIONS OF SOURCE/DATE ESTIMATION! Exiting early....")
	...
	for(i in 1:ncol(weights.mat)) weights.mat[,i]=weights.mat[,i]/sum(weights.mat[,i])
        ...
        tempweights=matrix(nrow=nrow(weights.mat)*nrow(weights.mat),ncol=length(donor.pops.all)*length(donor.pops.all))
        newdim=nrow(weights.mat)
        olddim=length(donor.pops.all)
        for(k in 1:length(donor.pops.all))
	{
	  for(l in 1:length(donor.pops.all))
	  {
		for(m in 1:nrow(weights.mat))
		{
			for(n in 1:nrow(weights.mat))
			{
				tempweights[(m-1)*newdim+n,(k-1)*olddim+l]=weights.mat[m,k]*weights.mat[n,l]
            		}
         	}	
          }
        }
...
#     }
     ...
     dists=as.double(names(x)[7:length(names(x))])
     times=dists
...
if ((prop.ind==1 || prop.continue.ind==1) && num.iterations>0)
{
	...
	if (is.na(fit.quality.orig)) fit.quality.orig=0
...
#            fit.quality2=get.fitted.results.1date.PC2[1]
	    ...
	    if (is.na(fit.quality.2date.PC2)) fit.quality.2date.PC2=0
	...
	if (sum(prop.val.est)==0) prop.val.est=0
	...
	if (length(donor.pops)==1)
	{
		print(paste("A SINGLE SURROGATE POP FITS MIXTURE AFTER ",a," ITERATIONS OF SOURCE/DATE ESTIMATION! Exiting early....",sep=''))
		write("### INFERRED SOURCES AND DATES ('best-guess' conclusion: Unknown",file=paste(save.file.props,prop.save.addon,prop.save.post,sep=''),ncolumns=1)
     		write("#######################################",file=paste(save.file.props,prop.save.addon,prop.save.post,sep=''),ncolumns=1,append=TRUE)
		write("### 1-DATE FIT EVIDENCE, DATE ESTIMATE, SINGLE BEST-FITTING DONORS",file=paste(save.file.props,prop.save.addon,prop.save.post,sep=''),ncolumns=1,append=TRUE)
     		write(c("gen.1date","proportion.source1","maxR2fit.1date","fit.quality.1event","fit.quality.2events","bestmatch.event1.source1","bestmatch.event1.source2","proportion.event2.source1","bestmatch.event2.source1","bestmatch.event2.source2"),file=paste(save.file.props,prop.save.addon,prop.save.post,sep=''),ncolumns=10,append=TRUE)
